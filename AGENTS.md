...
2. **课件不改壳**：只用壳里 7 种版式，只替换标记区内容和 `<title>`（课题名）；壳的内联库、主题 CSS、播放器 JS 一律不动。
3. **时间预算守恒**：各环节之和恒等于课时；新增环节必须显性扣减，禁止用"（含上）"藏时间；练:评 ≈ 2:1（1.5–2:1 皆可）；其他时长（20/60/90）按同比例缩放。
4. **提问不破坏节奏**：题目随计时亮出；冷叫三步（先问全班 → 给思考时间 → 最后点名）；冷场 5 秒教师自答兜底；一次只点 1–3 人，提问时间从"评"的预算里出。
5. **"咯噔"验收**：每个组块至少 1 个反直觉点，学员第一反应应是"我怎么没想到"→"原来如此"（先意外、后通透，缺一半即不合格）；哲学概念必须贴肉、一次课最多 2–3 个、每个一句话讲透；备用弹药只写进讲师提示和教案，不上屏。
